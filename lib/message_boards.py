...
def getThreadDetails(db, uid, access_level):
    error = None
    try:
        print("getting message threads")
        print(uid)
        cur = db.query("""select * from Threads where author = %s and view_level = %s""", [uid, access_level])
...
def postNewThread(db, form):
    ...
    try:
        ...
        # access_level = "f"
        db.query("""INSERT INTO Threads(author, thread_title, thread_description, postedat, view_level) VALUES(%s,%s,%s,NOW(),%s)""", [uid, threadTitle, description, access_level])
        db.conn.commit()
